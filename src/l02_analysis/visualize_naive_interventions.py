...
simulated_data_InfNaiveWe_mean = process_simulated_data(persuasion_values_grid, tgt_agt_centr_values_grid, error_method, variable, stat_method)
...
class ErrorBarPlot:
    ...
    def create_summary_bar_plot(self):
        ...
        if self.option == 'targeters':
            ax.set_ylabel('% Share of Naive Expectations', fontsize=16, fontweight='bold')
        else:
        ...
        # Save the figure
        folder_path = Path(__file__).resolve().parent.parent.parent / "reports" / "figures" / "Submission" / "naive_interventions"
        folder_path.mkdir(parents=True, exist_ok=True)  # Ensure the directory exists
            
        if self.option == "targeters":
            figure_path = folder_path / "errorbar_targeters_naive.png"
        else:
            figure_path = folder_path / "errorbar_std_naive.png"
...
summary_bar_plot = ErrorBarPlot(simulated_data_InfNaiveWe_mean, persuasion_values_grid, option="targeters", error_method="t-statistic")
...
class Custom_Boxplots:
    ...
    def create_plot(self):
        ...
        legend = ax.legend(handles, new_labels, loc='upper center', bbox_to_anchor=(0.5, 0.11), fancybox=True, shadow=True, ncol=len(new_labels))
        ...
        if self.option == "mean":
            figure_path = folder_path / "boxplots_targeters_naive.png"
        else:
            figure_path = folder_path / "boxplots_std_naive.png"
