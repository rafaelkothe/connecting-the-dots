...
class Deterministics_Plotter:
    ...
    def plot_figures(self):
        for network_type in self.df['network_type'].unique():
            ...
            folder_path = Path(__file__).resolve().parent.parent.parent / "reports" / "figures" / "CookieCutter2024" / "Deterministics" / today
...
file_path = Path(__file__).resolve().parent.parent.parent / "data" / "output_data" / "time_series" / date.today().strftime("%Y-%m-%d") / "time_series_data"
