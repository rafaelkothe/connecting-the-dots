...
def simulate_parameter_combinations(persuasion_values, tgt_agt_centr_values):
    ...
    for tgt_agt_centr, persuasion in zip(tgt_agt_centr_values_flat, persuasion_values_flat):
        results = multiple_simulation_runner(500, Params(capT=200, agent_number=100, tgt_agt_centr=tgt_agt_centr, degroot=True, intervention=True, target=False, persuasion=persuasion, topology='albert-barabasi'))
...
folder_path = Path(__file__).resolve().parent.parent.parent.parent / "data" / "output_data" / "naive_interventions" / today
